   ...
   if(isset($_POST['FB'])){
	...
	}else
	{
        ...
        if(isset($_POST['wpblp_new_post_status']))
                {
                    $status=$_POST['wpblp_new_post_status'];
                }else{
                    $status="Pending";
                }
                
         if(isset($_POST['wpblp_is_active']))
                {
                    $is_active=$_POST['wpblp_is_active'];
                }else{
                    $is_active=0;
         }
	...
	if(!($errorFlag==1)){
		global $wpdb;		
                ...
                if(!empty($attachmentfile)){
                    $values=array('business_name'=>$nm,'address'=>$addr,'city'=>$city,'zip'=>$zip,'lat'=>$lat,'glong'=>$long,'status'=>$status,'website'=>$web,'phone'=>$phno,'blog'=>$blog,'fb'=>$FB,'tw'=>$Twiiter,'googleplus'=>$GP,'linkedin'=>$linkedin,'description'=>$desc,'category'=>$special,'tags'=>$special,'modify_by'=>$user_ID,
		'created_by'=>$user_ID,'email'=>$email,'is_active'=>$is_active,'modify_date'=>$modify_date,'attachment'=>$attachmentfile);          
                }else{
		$values=array('business_name'=>$nm,'address'=>$addr,'city'=>$city,'zip'=>$zip,'lat'=>$lat,'glong'=>$long,'status'=>$status,'website'=>$web,'phone'=>$phno,'blog'=>$blog,'fb'=>$FB,'tw'=>$Twiiter,'googleplus'=>$GP,'linkedin'=>$linkedin,'description'=>$desc,'category'=>$special,'tags'=>$special,'modify_by'=>$user_ID,
		'created_by'=>$user_ID,'email'=>$email,'is_active'=>$is_active,'modify_date'=>$modify_date);
                }
                $where=array('id'=>$_POST['wpbdp_edit_listing']);
		...
		if($wpdb->update($wpdb->prefix.'wpbdp_listing',$values,$where)){
                        $listing_insert_id=$wpdb->insert_id;$sucssesMessage.="listing updated successfully";
     		$errorFlag=2;
